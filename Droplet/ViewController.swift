...
//  Droplet
//
//  Created by Razvan Julian on 22/06.2017.
...
class ViewController: UIViewController, GADBannerViewDelegate,MFMailComposeViewControllerDelegate, MFMessageComposeViewControllerDelegate{
  ...
  @IBOutlet weak var dropletLabel: UILabel!
  ...
  override func viewDidLoad() {
    ...
    prepareGame()
        ...
        let soundFileLocation = Bundle.main.path(forResource: "happy_preview_1", ofType: "mp3")
        
        
        
        do {
            
            try player = AVAudioPlayer(contentsOf: URL(fileURLWithPath: soundFileLocation!))
            player.play()
            
            timer = Timer.scheduledTimer(withTimeInterval: 35, repeats: true, block: { (timer) in
                self.player.play()
            })
    ...
    @IBAction func playOrStop(_ sender: Any) {
        ...
        if isPlayingSound {
            ...
            player.stop()
            timer.invalidate()
            soundButton.setImage(UIImage(named:"speaker-off"), for: [])
            ...
            soundButton.setImage(UIImage(named: "speaker-on"), for: [])
    ...
    @IBAction func changeTheme(_ sender: Any) {
        ...
        let newTheme = colors[Int(arc4random_uniform(UInt32(colors.count)))]
        playerView.backgroundColor = newTheme
        dropletLabel.textColor = newTheme
        startLabel.textColor = newTheme
        clockLabel.textColor = newTheme
        soundButton.tintColor = newTheme
        themeButton.tintColor = newTheme
        shareButton.tintColor = newTheme
        storeButton.tintColor = newTheme
    ...
    @IBAction func share(_ sender: AnyObject?) {
        ...
        let twitterAction = UIAlertAction(title: "Twitter", style: .default) { (alert) in
            ...
            if SLComposeViewController.isAvailable(forServiceType: SLServiceTypeTwitter) {
                ...
                twitter.setInitialText("Meet Droplet! Now available #free on the #AppStore! https://itunes.apple.com/us/app/droplet-find-the-escape/id1262422897?mt=8")
        ...
        let messageAction = UIAlertAction(title: "Message", style: .default) { (alert) in
            ...
            if MFMessageComposeViewController.canSendText() {
                ...
                message.subject = "Meet Droplet!"
                message.body = "Now available free on the AppStore! https://itunes.apple.com/us/app/droplet-find-the-escape/id1262422897?mt=8"
        ...
        let mailAction = UIAlertAction(title: "Mail", style: .default) { (alert) in
            ...
            if MFMailComposeViewController.canSendMail() {
                ...
                mail.setSubject("Meet Droplet!")
                mail.setMessageBody("Now available free on the AppStore! https://itunes.apple.com/us/app/droplet-find-the-escape/id1262422897?mt=8", isHTML: false)
...
fileprivate extension ViewController {
  func setupPlayerView() {
    ...
    playerView.backgroundColor = colors[Int(arc4random_uniform(UInt32(colors.count)))]
  ...
  func prepareGame() {
    ...
    dropletLabel.isHidden = false
    shareButton.isHidden = false
    soundButton.isHidden = false
    themeButton.isHidden = false
    storeButton.isHidden = false
    clockLabel.text = "Time: 00:00:000"
    playerView.backgroundColor = colors[Int(arc4random_uniform(UInt32(colors.count)))]
    dropletLabel.textColor = playerView.backgroundColor
    startLabel.textColor = playerView.backgroundColor
    clockLabel.textColor = playerView.backgroundColor
    
    
    let soundImage = UIImage(named: "speaker-on")
    let massageTintedImage = soundImage?.withRenderingMode(.alwaysTemplate)
    soundButton.setImage(massageTintedImage, for: .normal)
    soundButton.tintColor = playerView.backgroundColor
    
    let themeImage = UIImage(named: "color-circle")
    let mailTintedImage = themeImage?.withRenderingMode(.alwaysTemplate)
    themeButton.setImage(mailTintedImage, for: .normal)
    themeButton.tintColor = playerView.backgroundColor
    
    let shareImage = UIImage(named: "share")
    let shareTintedImage = shareImage?.withRenderingMode(.alwaysTemplate)
    shareButton.setImage(shareTintedImage, for: .normal)
    shareButton.tintColor = playerView.backgroundColor
    
    let storeImage = UIImage(named: "shopping-cart")
    let storeTintedImage = storeImage?.withRenderingMode(.alwaysTemplate)
    storeButton.setImage(storeTintedImage, for: .normal)
    storeButton.tintColor = playerView.backgroundColor
  ...
  func format(timeInterval: TimeInterval) -> String {
    ...
    return String(format: "Time: %02d:%02d.%03d", minutes, seconds, milliseconds)
  }
  ...
  func getGameOverTitleAndMessage() -> (String, String) {
    ...
    case 0..<10: return ("Another chance", "You need more experience!")
    case 10..<30: return ("One more time", "So close! You almost got it!")
    case 30..<60: return ("Another one", "That was right awesome!")
    case 60..<90: return ("Yeah!", "Congratulations, You won the game!")
    default:
      return ("Yeah!", "Congratulations, You won the game!")
    }
  }
